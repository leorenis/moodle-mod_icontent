...
switch ($action){
	case 'doubttutor':
		$urlparams += array('doubttutor' => $doubttutor, 'tab'=>$tab);
		$strheading = get_string('doubttutor', 'mod_icontent');
		break;
	case 'alldoubts':
		$urlparams += array('tab' => $tab);
...
$url = new moodle_url('/mod/icontent/doubts.php', $urlparams + array('page' => $page, 'perpage' => $perpage));
// Get sort value
...
$table->head  = array(get_string('fullname'), $strheading);
