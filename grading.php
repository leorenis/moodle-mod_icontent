...
$url = new moodle_url('/mod/icontent/grading.php', array('id'=>$id, 'action'=> $action, 'page' => $page, 'perpage' => $perpage));
// Get sort value
$sort = icontent_check_value_sort($sort);
// Get answers not evaluated
$attemptsusers = icontent_get_attempts_users_with_open_answers($cm->id, $sort, $page, $perpage);
$tattemtpsusers = icontent_count_attempts_users_with_open_answers($cm->id);
...
if($attemptsusers) foreach ($attemptsusers as $attemptuser){
	...
	$icontoevaluate = html_writer::link(
			 				new moodle_url('toevaluate.php',
			 					array('id' => $cm->id, 'userid' => $attemptuser->id, 'sesskey' => sesskey())
			 				),
...
// Info
echo html_writer::div(get_string('answersevaluatedinfo', 'mod_icontent'), 'alert alert-info');
// Show table
