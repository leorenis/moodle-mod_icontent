...
define('ICONTENT_QTYPE_MULTIANSWER', 'multianswer');
...
function icontent_get_questions_of_questionbank($coursecontext, $sort, $page = 0, $perpage = ICONTENT_PER_PAGE, $category = 0){
    ...
    ORDER BY {$sort}";
    $params = array($coursecontext, ICONTENT_QTYPE_ESSAY, ICONTENT_QTYPE_MATCH, ICONTENT_QTYPE_MULTICHOICE, ICONTENT_QTYPE_TRUEFALSE, ICONTENT_QTYPE_MULTIANSWER);
...
function icontent_count_questions_of_questionbank($coursecontext){
    ...
    $questions = $DB->get_record_sql("SELECT count(*) as total
        ...
        AND q.qtype IN (?,?,?,?,?)
        ",
        array($coursecontext, ICONTENT_QTYPE_ESSAY, ICONTENT_QTYPE_MATCH, ICONTENT_QTYPE_MULTICHOICE, ICONTENT_QTYPE_TRUEFALSE, ICONTENT_QTYPE_MULTIANSWER));
...
function icontent_get_attempt_summary_by_page($pageid, $cmid){
    ...
    AND pq.cmid = ?
    AND qa.userid = ?
    GROUP BY qa.timecreated limit 1;";
    // Get record
...
function icontent_make_questions_answers_by_type($question){
    ...
    switch ($question->qtype){
        case ICONTENT_QTYPE_MULTIANSWES:
            ...
            require_once($_SERVER['DOCUMENT_ROOT'] . '/question/previewlib.php');
            require_once($CFG->dirroot . '/mod/quiz/locallib.php');
            //$question->questiontext = preg_replace('/\{\#(\d+)\}/', 'radio$1', $question->questiontext);
            //$question->questiontext = preg_replace('/\{\#(\d+)\}/', '<div><input type="radio"></div>', $question->questiontext);
            /*$arrParts = explode('<li', $question->questiontext);
            foreach($arrParts as $row)
            {
          ...
          //  $attempts = get_records_menu_attempts('icontent_question_attempts', array('cmid' => $cm->id, 'pagesquestionsid' => $question->qpid, 'questionid' => $question->qid, 'userid' => $USER->id), '', 'sub,answertext');
            ...
            $slot = $quba->add_question($objQuestion, $options->maxmark);

            if ($objQuestion->length) {
                $displaynumber = '1';
            } else {
                $displaynumber = 'i';
            }
                ...
                if(preg_match_all('/<select [^\>]*name=\"([A-Za-z0-9\_\-]+\_multianswer\_(\d+))\"[^\>]*>(.*?)<\/select>/mi', $context, $matches2))
                {
                    foreach ($matches2[3] as $key => $val)
                    {
                        if(preg_match_all('/<option value=\"(\d+)\"/im', $val, $mat2))
                        {
                            ...
                            {
                                if(isset($attempts[$matches2[2][$key]])&&$attempts[$matches2[2][$key]] == $val2)
                                {
            ...
            $questionanswers .= html_writer::div($context, 'questiontext');
...
function get_records_menu_attempts($table, array $values) {
