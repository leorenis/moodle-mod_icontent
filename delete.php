...
$PAGE->set_url('/mod/icontent/delete.php', ['id' => $id, 'pageid' => $pageid]);
...
if ($confirm) {
    ...
    // Delete records.
    $DB->delete_records('icontent_pages_displayed', ['pageid' => $page->id]);
    // Get page questions for remove items attempts.
    $pagequestions = $DB->get_records('icontent_pages_questions', ['pageid' => $page->id], null, 'id, pageid, cmid');
    if ($pagequestions) {
        foreach ($pagequestions as $pagequestion) {
            $DB->delete_records('icontent_question_attempts', ['pagesquestionsid' => $pagequestion->id]);
        }
    }
...
echo '<br />';
$continue = new moodle_url('/mod/icontent/delete.php', ['id' => $cm->id, 'pageid' => $page->id, 'confirm' => 1]);
$cancel = new moodle_url('/mod/icontent/view.php', ['id' => $cm->id, 'pageid' => $page->id]);
